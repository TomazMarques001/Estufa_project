...
EDGE-APP: realiza a leitura dos tópicos no MQTT via Nodered, que por sua vez comunica e realiza as regras de negocio entre o broker e as demais aplicações contidas no compose, de forma a centralizar regras de negocio, monitoramento, e aquisição de informações.<br><br>
O edge connector disponibiliza as informações captadas via protocolos OPC UA e MQQT, pela arquitetura padrão nesses casos foi usado o protocolo MQTT pra disponiblizar as informações via BROKER, e o OPC UA pra receber informações pertinentes a lógica do CLP.<br>
